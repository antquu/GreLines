  ...
  return (
    <div className="fixed z-50 top-4 left-4">
      ...
      >
        ...
        {isFocused && (searchQuery.trim() !== '' ? matchedStops.length > 0 : searchHistoryItems.length > 0) && (
            ...
            {searchQuery.trim() !== '' ? (
              matchedStops.map((stop) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">{stop.city || unknownCityLabel}</div>
                </button>
              ))
            ) : (
              searchHistoryItems.map((historyItem, index) => (
                <button
                  ...
                  className="w-full text-left px-3 py-2 hover:bg-gray-100 dark:hover:bg-slate-800 transition"
                >
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{historyItem}</div>
                  <div className="text-xs text-gray-500 dark:text-gray-400">{recentSearchLabel}</div>
                </button>
              ))
            )}
